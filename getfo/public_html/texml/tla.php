...
function tla_ads() {
	...
	$LOCAL_XML_FILENAME = "local_150507.xml";
	...
	if( filemtime($LOCAL_XML_FILENAME) < (time() - 3600) || filesize($LOCAL_XML_FILENAME) < 20) {
		...
		tla_updateLocalXML("http://www.text-link-ads.com/xml.php?inventory_key=JU8VWDZ0HLBEES3F646A&referer=" . urlencode($request_uri) .  "&user_agent=" . urlencode($user_agent), $LOCAL_XML_FILENAME, $CONNECTION_TIMEOUT);
	}
	...
	if ( is_array($arr_xml) ) {
		echo "\n<ul style=\"list-style: none; width: 100%; border: 1px solid #000000; border-spacing: 0px; background-color: #F0F0F0; overflow: hidden; margin: 0; padding: 0;\">\n";
		for ($i = 0; $i < count($arr_xml['URL']); $i++) {
			if( isset($arr_xml['PostID'][$i]) && $arr_xml['PostID'][$i] > 0 ) continue;
			echo "<li style=\"padding: 0; clear: none; width: 100%; float: left; display: inline; margin: 0;\"><span style=\"margin: 0; color: #000000; width: 100%; padding: 3px; display: block; font-size: 12px;\">".$arr_xml['BeforeText'][$i]." <a style=\"color: #000000; font-size: 12px;\" href=\"".$arr_xml['URL'][$i]."\">".$arr_xml['Text'][$i]."</a> ".$arr_xml['AfterText'][$i]."</span></li>\n";
		}
		echo "</ul>";
	}
